...
对比三个版本的性能：
1. inject_features_legacy (iterrows)
2. inject_features (向量化)
3. inject_features_parallel (多进程)
4. inject_features_cached (缓存)
"""
import time
import pandas as pd
import numpy as np
...
for config in test_configs:
    ...
    if results['parallel']:
        ...
        print(f"  并行处理:       {results['parallel']:.2f}s (提升 {speedup_p:.2f}x)")
    if results['cached']:
