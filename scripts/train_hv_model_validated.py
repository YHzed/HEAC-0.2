...
使用:
    python scripts/train_hv_model_validated.py
...
def train_hv_model(data_path, features_path, output_dir='models/validated'):
    """训练HV预测模型"""
    
    print("=" * 80)
    print("🚀 HV模型训练脚本（增强验证版）")
    print("=" * 80)
    ...
    print(f"   HV范围: [{y.min():.1f}, {y.max():.1f}], 均值: {y.mean():.1f}")
    
    # 交叉验证
    print("\n[4/6] 交叉验证训练...")
    
    # 使用XGBoost（移除early_stopping，因cross_val_predict不支持）
    model = XGBRegressor(
        n_estimators=1000,
        learning_rate=0.05,
        max_depth=6,
    ...
    print("   执行5-Fold交叉验证...")
    y_pred_cv = cross_val_predict(model, X, y, cv=cv, n_jobs=1)  # 使用单线程避免pickle问题
    ...
    # 保存模型
    model_path = output_path / 'hv_validated_model.pkl'
    joblib.dump(model, model_path)
    print(f"   ✅ 模型: {model_path}")
    
    # 保存特征列表
    features_output_path = output_path / 'hv_feature_list.json'
    ...
    report_path = output_path / 'hv_training_report.json'
...
def main():
    parser = argparse.ArgumentParser(description='训练HV预测模型（增强验证版）')
