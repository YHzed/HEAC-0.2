...
class ModelXAdapter:
    ...
    特征生成遵循严格的依赖链：
    Layer 1 (基础): Matminer化学特征, Binder_Wt_Pct, Grain_Size_um
    ...
    def extract_modelx_features(
        ...
        features['Binder_Wt_Pct'] = binder_wt_pct
    ...
    def _calculate_lattice_mismatch(pred_lattice_fcc: float, ceramic_type: str = 'WC') -> float:
        ...
        # 陶瓷相晶格参数
        CERAMIC_LATTICE = {
            'WC': 2.906,  # Hexagonal a
            'TiC': 4.32,  # FCC a
            'TiN': 4.24,
            'VC': 4.16
        }
        
        a_ceramic = CERAMIC_LATTICE.get(ceramic_type, 2.906)
    ...
    def _get_electronegativity(element: str) -> float:
        ...
        return electroneg.get(element, 1.5)
